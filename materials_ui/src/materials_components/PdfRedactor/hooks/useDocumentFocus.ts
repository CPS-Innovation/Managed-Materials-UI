...
export const useDocumentFocus = (p: { containerRef: RefObject<HTMLElement | null> }) => {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      currentWordIndex.current = clamp(currentWordIndex.current + step, 0, words.length - 1);
...
const collectWordPositions = (container: HTMLElement): WordPosition[] => {
  ...
  const spans = Array.from(textLayers).flatMap((tl) => getOrderedTextSpans(tl.children));
  return spans.flatMap((span) =>
    getWordStartingIndices(span.textContent ?? '').map((offset) => ({ span, offset })),
  );
};

const placeCaretAt = ({ span, offset }: WordPosition, container: HTMLElement) => {
