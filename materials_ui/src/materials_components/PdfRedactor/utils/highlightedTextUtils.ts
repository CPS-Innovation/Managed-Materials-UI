...
export const getPdfCoordPairsOfHighlightedText = (p: { pdfPageRect: DOMRect; scale: number }) => {
  ...
  const coordPairs = mergedRects.map((rect) => {
    if (rect.width < MIN_REDACTION_SIZE_PX || rect.height < MIN_REDACTION_SIZE_PX) return;
    ...
    const newRedaction: TCoordPair = { x1: coord1.x, y1: coord1.y, x2: coord2.x, y2: coord2.y };
    return newRedaction;
  });
