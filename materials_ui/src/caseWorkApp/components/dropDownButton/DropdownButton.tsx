...
export const DropdownListItem = (initProps: TButtonProps & { borderBottom: boolean }) => {
  const { borderBottom, ...props } = initProps;
  return (
    <div style={{ whiteSpace: 'nowrap', borderBottom: borderBottom ? 'solid 2px black' : '' }}>
      <Button
        style={{ width: '100%', textAlign: 'right', boxShadow: borderBottom ? undefined : 'unset' }}
        {...props}
      />
    </div>
  );
...
export const DropdownButton2 = (p: {
  ...
  return (
    <span style={{ position: 'relative' }}>
      <Button aria-label={p.ariaLabel} ref={dropDownBtnRef} onClick={() => p.setIsOpen(!p.isOpen)}>
        <div style={{ display: 'flex', gap: '4px' }}>
          <div>{p.ButtonContent}</div>
          <DownArrowIcon color="#1d70b8" rotateDegrees={p.isOpen ? 180 : 0} scale={0.75} />
        </div>
      </Button>
      {p.isOpen && (
        <div
          ref={dropDownPanelRef}
          style={{ position: 'absolute', top: '100%', right: '0', zIndex: 1007 }}
        >
          <div style={{ background: '#fff', filter: 'drop-shadow(0 1px 0.15rem #000)' }}>
            {p.children}
          </div>
        </div>
      )}
...
export const DropdownButton: React.FC<DropdownButtonProps> = ({
  ...
  useGlobalDropdownClose(dropDownBtnRef, panelRef, setButtonOpen, '#dropdown-panel');
    ...
    <div className={classes.dropDownButtonWrapper}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: '4px' }}>
          {name && (
            <span style={{ fontWeight: '700' }} className={classes.dropdownBtnName}>
              {name}
            </span>
          )}
          <DownArrowIcon color="#1d70b8" rotateDegrees={buttonOpen ? 180 : 0} scale={iconScale} />
        </div>
      </LinkButton>
