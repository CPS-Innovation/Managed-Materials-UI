...
test.describe('PCD Review', () => {
  test('T-001: page loads Initial Review and displays all main sections', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Case Headline ' })).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Charging Decision & Advice' })).toBeVisible();
    await expect(
      page.getByRole('heading', { name: 'Further action agreed for codes A, B, B2, H, I, J' }),
    ).toBeVisible();

    // Table headings
    await expect(page.getByRole('columnheader', { name: 'Suspect' })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: 'Charging code' })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: 'Advice' })).toBeVisible();
